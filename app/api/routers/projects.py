...
@router.post("/", response_model=ProjectOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_api_key)])
async def create_project(payload: ProjectCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create_project(db, payload)

@router.get("/{project_id}", response_model=ProjectOut, dependencies=[Depends(verify_api_key)])
...
@router.put("/{project_id}", response_model=ProjectOut, dependencies=[Depends(verify_api_key)])
...
@router.post("/{project_id}/tasks", response_model=TaskOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_api_key)])
...
@router.get("/{project_id}/tasks", response_model=list[TaskOut], dependencies=[Depends(verify_api_key)])
